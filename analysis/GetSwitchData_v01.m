function [Xoutcomes,Xrt,blockdata] = GetSwitchData_v01(bhv)
...
for s = 1:numel(SessionIDs)
    ...
    for b = 1:numel(Xtrials)
        ...
        IDshift  = sdata.DimNum(Xtrials(b)-1) == sdata.DimNum(Xtrials(b));
        
         
        if IDshift ==1 & sdata.DimNum(Xtrials(b)) ==1
            StateTransType = 1; % green to green
        elseif IDshift ==1 & sdata.DimNum(Xtrials(b)) ==2
            StateTransType = 2;      % pink to pink
        elseif IDshift ==0 & sdata.DimNum(Xtrials(b)) ==1
            StateTransType = 3; % pink to green
        elseif IDshift ==0 & sdata.DimNum(Xtrials(b)) ==2
